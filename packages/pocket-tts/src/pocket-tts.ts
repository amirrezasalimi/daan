...
export function runMimiStreamingMultiheadAttention(
  ...
  if (isPrefill) {
    ...
    const cacheMask = np.arange(capacity).reshape([-1, 1, 1]).less(kvCacheLen.ref);
...
export function runSEANetEncoder(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    [x, states] = runSEANetResnetBlock(model[idx] as SEANetResnetBlock, states, x);
    tree.dispose(states);
    idx++;
    // ELU
    x = nn.elu(x);
...
export function createSEANetDecoderState({ model }: SEANetDecoder): SEANetDecoderState {
  return {
    conv1: createConv1dState(model[0].conv),
    blocks: [
      {
        convtr: createConvTranspose1dState(model[2].convtr, 6),
        res: [createConv1dState(model[3].block[1].conv), createConv1dState(model[3].block[3].conv)],
      },
      {
        convtr: createConvTranspose1dState(model[5].convtr, 5),
        res: [createConv1dState(model[6].block[1].conv), createConv1dState(model[6].block[3].conv)],
      },
      {
        convtr: createConvTranspose1dState(model[8].convtr, 4),
        res: [createConv1dState(model[9].block[1].conv), createConv1dState(model[9].block[3].conv)],
      },
    ],
...
export const runSEANetDecoder = jit(function runSEANetDecoder(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    idx++;
    // ResBlock
    [x, blockState.res] = runSEANetResnetBlock(model[idx] as SEANetResnetBlock, blockState.res, x);
    idx++;
  }
...
export function runMimiEncode(
  ...
  for (const layer of encoderTransformer) {
    ...
    [x, kvCache] = runStreamingTransformerLayer(layer, kvCache, x, offset.ref, 0, {
      context: 250,
      numHeads: 8,
    });
...
export function runMimiDecode(
  ...
  { kvCaches, kvCacheLen, offset, initialConvState, seanetStates }: MimiDecodeState,
  ...
  for (let i = 0; i < decoderTransformer.length; i++) {
    ...
    [x, kvCaches[i]] = runStreamingTransformerLayer(layer, kvCaches[i], x, offset, kvCacheLen, {
      context: 250,
      numHeads: 8,
    });
  }
...
export function runTimestepEmbedder({ mlp, freqs }: TimestepEmbedder, t: np.Array): np.Array {
...
export function runRMSNorm({ alpha }: RMSNorm, x: np.Array, eps: number = 1e-5) {
...
export function createConv1dState({ weight }: Conv1d, stride: number = 1): np.Array {
  return np.zeros(
    [
      1, // batch size
...
export function fromSafetensors(file: safetensors.File, dtype: np.DType = np.float32): PocketTTS {
