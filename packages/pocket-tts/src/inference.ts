  ...
  try {
    ...
    for (let step = 0; step < 1000; step++) {
      ...
      if (eosStep !== null && step >= eosStep + framesAfterEos) {
        console.log(`Generation ended at step ${step}, ${framesAfterEos} frames after EOS.`);
        latent.dispose();
        break;
      }
      ...
      console.log(`Generated step ${step} in ${(timestamp - lastTimestamp).toFixed(1)} ms`);
      lastTimestamp = timestamp;

      const mimiInput = latent.ref.mul(model.flowLM.embStd.ref).add(model.flowLM.embMean.ref);

      const [audio, newMimiState] = runMimiDecode(tree.ref(model.mimi), mimiState, mimiInput);
      ...
      audioPromise = (async () => {
        ...
        const windowElapsedSeconds =
          recentFrameDurationsMs.reduce((sum, duration) => sum + duration, 0) / 1000;
        const framesPerSecond = recentFrameDurationsMs.length / windowElapsedSeconds;
        onProgress?.({
          framesGenerated,
